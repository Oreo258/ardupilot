...
{
    uint8_t buf[8] = {0xFF, 0x01, 0x4E, 0x24, 0x0E, 0x51, 0x43, 0xF0};
    uint8_t num_drivers = AP::can().get_num_drivers();
    for (uint8_t i = 0; i < num_drivers; i++) {
        if(AP::can().get_driver_type(i) == AP_CANManager::Driver_Type_GeneralCAN) {
            AP_GeneralCAN *ap_gcan = AP_GeneralCAN::get_gcan(i);
            if (ap_gcan != nullptr) {
                if (!ap_gcan->write_frame(0x401, buf, 8)) {
                    return false;
                 }
            }
        }
